...
def Create_Product(Username,product_json_file,product_ID,product_title,product_type,price_per_day,total_stock_available):
...
def Add_item_to_cart(Username,product_ID,Quantity,gamers_json_file,booking_start_date,booking_end_date,products_json_file):
    ...
    for i in range(len(content1)):
        if content1[i]["Product ID"] == product_ID:
            if content1[i]["Total Stock Available"] >= Quantity:
                # content1[i]["Total Stock Available"] -= Quantity ( reduce Quantity only for order)
                for j in range(len(content)):
                    if content[j]["Username"] == Username:
                        values = {"Product ID": product_ID,
                                  "Quantity": Quantity,
                                  "Price": content1[i]["Price Per Day"],
                                  "Booking Start Date": booking_start_date,
                                  "Booking End Date": booking_end_date}
                        content[j]["Cart"].append(values)
                file.seek(0)
                file.truncate()
                json.dump(content, file, indent=4)
                file.close()
...
def Place_order(Username,gamers_json_file,Order_Id,orders_json_file,products_json_file):
    ...
    for i in range(len(content3)):
        Buyingit.append(content3[i]["Cart"])
        list = content3[i]["Cart"]
        for i in range(len(list)):
            Cost= days_between((list[i]["Booking End Date"]),(list[i]["Booking Start Date"]))
            Total_amount = Cost*list[i]["Price"]

    vari = {"Order ID": Order_Id,
            "Ordered by": Username,
            "Items": Buyingit,
